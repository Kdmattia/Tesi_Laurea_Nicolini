...
// Compute the base path of this script at runtime so all asset paths
// (Monaco, ACE, ANTLR runtime, worker) resolve correctly regardless
// of the deployment location or port.
 ...
 * - A collapsible parse tree is displayed below the editor as a syntax-highlighted
 *   XML document, with rule nodes as XML tags and token values as text content.
 * - The generated XML parse tree can be downloaded as an `.xml` file via the
 *   Download XML button.
 *
 * ```html
...
export class PbDslEditor extends pbMixin(LitElement) {
  ...
  static get properties() {
    return {
      ...
      /**
       * List of parse errors from the last Web Worker parse result,
       * displayed in the error panel below the Monaco editor.
       */
      _parseErrors: { type: Array },
    };
  }
  ...
  _injectAceStyles() {
    ...
    aceCss.href = `${SCRIPT_BASE}css/ace/ace.css`;
    const themeCss = document.createElement('link');
    themeCss.rel  = 'stylesheet';
    themeCss.href = `${SCRIPT_BASE}css/ace/theme/textmate.css`;
  ...
  _initAceEditor() {
    ...
    ace.config.set("basePath",           `${SCRIPT_BASE}ace-master/src`);
    ace.config.set("themePath",          `${SCRIPT_BASE}ace-master/src/theme`);
    ace.config.set("modePath",           `${SCRIPT_BASE}ace-master/src/mode`);
    ace.config.set("workerPath",         `${SCRIPT_BASE}ace-master/src/worker`);
  ...
  _injectMonacoStyles() {
    ...
    monacoCss.href = `${SCRIPT_BASE}monaco-editor/min/vs/editor/editor.main.css`;
  ...
  _loadMonaco() {
    return new Promise(resolve => {
      ...
      window.MonacoEnvironment = {
        getWorkerUrl: () =>
          `${SCRIPT_BASE}monaco-editor/min/vs/base/worker/workerMain.js`
      };
      const script = document.createElement("script");
      script.src = `${SCRIPT_BASE}monaco-editor/min/vs/loader.js`;
      script.onload = () => {
        window.require.config({
          paths: { vs: `${SCRIPT_BASE}monaco-editor/min/vs` }
        });
  ...
  _initMonacoEditor() {
    ...
    this._worker.onmessage = (event) => {
      ...
      if (msg.type === "parsed") {
        ...
        this._parseErrors = msg.errors || [];
        this._showErrors(this._parseErrors);
      }
    };
  ...
  _downloadXml() {
    ...
    const filename = (this.selectedDsl && this.selectedDsl.name
      ? this.selectedDsl.name
      : 'parse-tree') + '.xml';
  ...
  _loadDsls() {
    fetch(`${window.location.origin}/exist/apps/tei-publisher/api/dsls`)
      .then(r => r.json())
  ...
  _selectDsl(e) {
    ...
    this._worker.postMessage({
      ...
      baseUrl:  `${window.location.origin}/exist/rest/db/apps/tei-publisher/generated/`,
      antlrUrl: `${SCRIPT_BASE}antlr4/runtime/JavaScript/dist/antlr4.js`
    });
  ...
  async loadDsl(id) {
    const response = await fetch(
      `${window.location.origin}/exist/apps/tei-publisher/api/dsls/${id}`
    );
  ...
  async _saveDsl() {
    ...
    try {
      ...
      const response = await fetch(
        `${window.location.origin}/exist/apps/tei-publisher/api/dsls`,
  ...
  render() {
    return html`
      ...
      ${this._parseErrors && this._parseErrors.length > 0 ? html`
        <div class="error-panel">
          <div class="error-panel-header">
            ⚠ ${this._parseErrors.length} error${this._parseErrors.length > 1 ? 's' : ''}
          </div>
          ${this._parseErrors.map(err => html`
            <div class="error-item">
              <span class="error-location">Line ${err.startLine}, Col ${err.startCol + 1}</span>
