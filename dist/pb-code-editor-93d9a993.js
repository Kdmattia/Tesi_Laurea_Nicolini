import { j as templatize, m as modelForElement, P as Polymer, h as html, k as resolveUrl, B as Base, d as dom, a as IronButtonState, I as IronControlState, l as flush, n as DomModule, D as Debouncer, o as idlePeriod, e as enqueueDebouncer, p as get } from './paper-inky-focus-behavior-b5e152b3.js';
import { a as IronResizableBehavior } from './paper-checkbox-d6ebdce3.js';
import { d as directive, N as NodePart, i as isPrimitive, p as pbMixin, _ as _objectSpread2, h as html$1, c as css$1, L as LitElement } from './pb-mixin-fdfbc739.js';
...
$_documentContainer$6.innerHTML = `<dom-module id="paper-item-shared-styles">
  <template>
    <style>
      :host, .paper-item {
        display: block;
...
let chrome = !edge && /Chrome\/(\d+)/.exec(userAgent);
let chrome_version = chrome && +chrome[1];
...
function widgetTopHeight(lineObj) {
  let {
    ...
    height = 0;
  if (widgets) for (let i = 0; i < widgets.length; ++i) if (widgets[i].above) height += widgetHeight(widgets[i]);
  return height;
}
...
function maybeScrollWindow(cm, rect) {
  ...
  if (rect.top + box.top < 0) doScroll = true;else if (rect.bottom + box.top > (doc.defaultView.innerHeight || doc.documentElement.clientHeight)) doScroll = false;
...
class NativeScrollbars {
  ...
  enableZeroWidthBar(bar, delay, type) {
    ...
    function maybeDisable() {
      ...
      if (elt != bar) bar.style.visibility = "hidden";else delay.set(1000, maybeDisable);
    }
...
function onScrollWheel(cm, e) {
  ...
  if (chrome && chrome_version == 102) {
    if (cm.display.chromeScrollHack == null) cm.display.sizer.style.pointerEvents = "none";else clearTimeout(cm.display.chromeScrollHack);
...
function leftButtonDown(cm, pos, repeat, event) {
  if (ie) setTimeout(bind(ensureFocus, cm), 0);else cm.curOp.focus = activeElt(root(cm));
...
function handlePaste(e, cm) {
  ...
  if (pasted) {
    e.preventDefault();
    if (!cm.isReadOnly() && !cm.options.disableInput && cm.hasFocus()) runInOp(cm, () => applyTextInput(cm, pasted, 0, null, "paste"));
    return true;
  }
}
...
class ContentEditableInput {
  ...
  receivedFocus() {
    let input = this;
    if (this.selectionInEditor()) setTimeout(() => this.pollSelection(), 20);else runInOp(this.cm, () => input.cm.curOp.selectionChanged = true);
    function poll() {
...
//# sourceMappingURL=pb-code-editor-93d9a993.js.map
